	...
	<p>Timestamps are in server time. <span id="irclog-toggle-ircstatus-span" style="display:none"><input id="irclog-toggle-ircstatus" type="checkbox" value=""><label for="irclog-toggle-ircstatus">Show join/quit messages</label></span></p>
		...
		for ($i = 0; $i < count($lines); $i++) {
			...
			if (count($matches) >= 4) {
				...
				$line = preg_replace(
						'!(http|https)://(|www\.)(faiumoni.de|stendhalgame.org|arianne.sf.net|arianne-project.org|arianne.sourceforge.net|sourceforge.net|sf.net|download.oracle.com|libregamewiki.org|freesound.org|opengameart.org|openclipart.org)(/[^ ]*)?!',
